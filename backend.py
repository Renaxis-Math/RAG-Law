...
# %%
# Use the exact DB_CONNECTION from .env
...
# Create a PGVector-backed store for the Insurance Code
...
def parse_pdf(path: str, source: str) -> List[Document]:
    """
    Load each page of a PDF as a Document, tagging with 'source'.
    """
    docs: List[Document] = []
    try:
...
1) **Vectorstore** – answer from local Insurance Code docs ({vectorstore_summary}).
2) **Websearch** – answer from external sources (case law, commentary, updates).
3) **Chitter-Chatter** – for off-topic or casual queries.

Return a JSON object: {{ "Datasource": "<Vectorstore|Websearch|Chitter-Chatter>" }}
""")
...
retriever = (
    multi_chain
    | insurance_vector_store.as_retriever(
        search_type="mmr",
...
def rewrite_query(state):
    ...
    return {"question": out["rewritten_question"], "original": state["question"]}
...
async def grade_docs(state):
    ...
    verdict = "pass" if len(passed) >= len(state["documents"])/2 else "fail"
    return {"documents": passed, "checker": verdict}
...
def decide_to_generate(state):
    return "Websearch" if state["checker"]=="fail" else "generate"

def check_final(_):
    return END  # placeholder if you later insert hallucination/verification nodes

# Assemble StateGraph
workflow = StateGraph(GraphState)
workflow.add_node("Websearch", web_search_node)
workflow.add_node("Retrieve", document_retriever)
...
workflow.set_conditional_entry_point(
    route_question,
    {"Vectorstore":"Retrieve", "Websearch":"Websearch", "Chitter-Chatter":"Chitter"}
)
workflow.add_edge("Retrieve", "Grade")
workflow.add_conditional_edges("Grade", decide_to_generate, {"Websearch":"Websearch", "generate":"Generate"})
workflow.add_edge("Websearch", "Generate")
workflow.add_edge("Generate", "Chitter")
